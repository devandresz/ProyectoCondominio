// ============================================================
// 📁 RUTA: frontend/src/paginas/modulos/ModuloAreasSociales.jsx
// ============================================================
...
export default function ModuloAreasSociales() {
	...
	const guardarEdicion = async (e) => {
		...
		try {
			...
			await cargarAreas();
			cerrarModal();
			toast.success('Área social actualizada exitosamente');
		} catch (err) {
			console.error(err);
			setError('No se pudo actualizar el área social.');
			toast.error('Error al actualizar el área social');
		} finally {
			setGuardando(false);
		}
	};
	...
	const cambiarEstado = async (area) => {
		try {
			setError('');
			await areasSocialesApi.cambiarEstado(area.ID_AREA, area.ACTIVO ? 0 : 1);
			await cargarAreas();
			toast.success('Estado del área modificado');
		} catch (err) {
			console.error(err);
			setError('No se pudo cambiar el estado del área social.');
			toast.error('Error al cambiar el estado del área');
		}
	};
										...
										<td className="px-3 py-3">
											...
											<span
												className={`rounded-full px-3 py-1 text-xs font-semibold ${
													area.ACTIVO ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
												}`}
											>
